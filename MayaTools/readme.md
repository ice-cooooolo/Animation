...
#### ❓ Q3 (管线思维): "为什么你要把截图放在 `_versions` 文件夹里，而不是直接存在 Maya 文件里？"
...
- **✅ 满分回答**：
  ...
  > "为了**解耦**和**性能**。
  >
  > 1. **性能**：如果在 UI 上浏览版本时，每次都要去解析巨大的 `.ma` 文件来获取缩略图，速度会慢到无法接受。独立的 `.jpg` 文件读取速度极快。
  > 2. **通用性**：`.jpg` 可以被网页、ShotGrid 或文件管理器直接预览，不需要安装 Maya 就能看。
  > 3. **Git/SVN 友好**：二进制数据（图片）和文本数据（代码/JSON）分离，有利于版本控制系统的管理。"

#### ❓ Q4 (异常处理): "如果在保存版本时，截图成功了，文件复制成功了，但是写 JSON 时程序崩溃了（比如断电），怎么办？"



- **🕵️‍♂️ 考察点**：**数据一致性 (Data Consistency)** 和 **事务 (Transaction)** 概念。

- **现状**：你的代码是分步执行的，确实可能出现“文件有了，但 JSON 里没记录”的**孤儿文件**情况。

- **✅ 满分回答**：

  > "这确实是一个数据一致性问题。 为了解决这个问题，我可以引入简单的**事务回滚 (Rollback)** 机制： 在 `try...except` 块中执行操作。如果最后一步（写 JSON）报错了，我在 `except` 里执行清理操作——把刚才复制的 `.ma` 文件和 `.jpg` 截图**删掉**。 这样保证了操作要么全部成功，要么全部失败（All or Nothing），不会留下脏数据。"